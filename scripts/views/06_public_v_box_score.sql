-- ================================================================
-- v_box_score — PUBLIC SCHEMA
-- Player stats with resolved names from players table
...
SELECT
  ...
  -- Resolved player name (prefer players.full_name, then player_stats fields)
  COALESCE(
    NULLIF(p.full_name, ''),
    NULLIF(ps.full_name, ''),
...
LEFT JOIN public.players p ON ps.player_id = p.id
WHERE ps.game_key IS NOT NULL;

GRANT SELECT ON public.v_box_score TO anon;
GRANT SELECT ON public.v_box_score TO authenticated;
