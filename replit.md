...
- **UI/UX Decisions**: Emphasizes a clean, professional design with a focus on readability and intuitive navigation. Utilizes a softer orange color palette (orange-50/100/300/400/700/800) for better aesthetics, with white backgrounds and soft orange borders (border-orange-200/300) for cards. Playful micro-interactions, hover animations, and smooth transitions are incorporated.
    ...
    - **League Page Branded Background**: League slug pages extract the dominant color from the banner image (falling back to logo) and apply a subtle gradient tint to the page background. Light mode uses ~8% opacity at the top fading to transparent; dark mode uses ~12% opacity. Colors are cached in localStorage by league slug for 7 days. Gracefully falls back to the default cream/dark background if extraction fails. Uses `useLeagueBranding` hook (`client/src/hooks/useLeagueBranding.ts`).
