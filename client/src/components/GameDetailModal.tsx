...
export default function GameDetailModal({ gameId, isOpen, onClose }: GameDetailModalProps) {
  ...
  // Load events when Summary, Feed, or Shot Chart tab is activated, or when gameId changes while on those tabs
  useEffect(() => {
    if ((activeTab === "summary" || activeTab === "feed" || activeTab === "shotchart") && !eventsLoaded && gameId) {
      fetchLiveEvents();
    }
              ...
              <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                ...
                <TabsList className="w-full flex flex-nowrap overflow-x-auto scrollbar-none bg-white dark:bg-neutral-900 border-b border-gray-200 dark:border-neutral-700 sticky top-0 z-20 rounded-none h-auto p-0 justify-start">
                  ...
                  >
                    ...
                    <span className="text-sm font-medium">Play by Play</span>
                  </TabsTrigger>
                  <TabsTrigger 
                    value="shotchart" 
                    className="flex-shrink-0 px-4 md:px-6 py-3 rounded-none data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-orange-500 data-[state=active]:text-orange-600"
                ...
                <TabsContent value="summary" className="mt-4 space-y-4">
                  ...
                  {!eventsLoading && liveEvents.length > 0 && gameInfo && (
                    <div className="bg-white dark:bg-neutral-800 rounded-lg p-4 border border-gray-200 dark:border-neutral-700">
                      <h3 className="text-sm font-semibold text-slate-600 dark:text-slate-300 mb-3 uppercase tracking-wide">Game Flow</h3>
                      <GameFlowSummary events={liveEvents as any} homeTeam={gameInfo.teams[0] || "Home"} awayTeam={gameInfo.teams[1] || "Away"} />
                    </div>
                  )}
                </TabsContent>
                ...
                <TabsContent value="feed" className="mt-4 space-y-4">
                  <PlayByPlay
                    ...
                    emptyMessage="No play-by-play data available for this game."
                    describeEvent={(event, previousScoredEvent) => generatePlayCaption(event, previousScoredEvent)}
                  />
                </TabsContent>
